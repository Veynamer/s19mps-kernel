...
static int psc2965_charger_hw_init(struct psc2965_charger_info *info)
{
	...
	if (info->role == PSC2965_ROLE_MASTER_DEFAULT) {
		ret = psc2965_charger_set_ovp(info, PSC2965_FCHG_OVP_6V);
		if (ret) {
			dev_err(info->dev, "set psc2965 ovp failed\n");
			return ret;
		}
	}
...
/*
		...
		if (ret) {
			dev_err(info->dev, "enable psc2965 charge failed\n");
			return ret;
		}
*/

		ret = psc2965_update_bits(info, PSC2965_REG_1,
					  PSC2965_REG_CHG_MASK,
					  0x1 << PSC2965_REG_CHG_SHIFT);
		if (ret) {
			dev_err(info->dev, "enable psc2965 charge en failed\n");
			return ret;
		}
	}
...
/*
		ret = regmap_update_bits(info->pmic, info->charger_pd,
					 ...
					 info->charger_pd_mask);
		if (ret)
			dev_err(info->dev, "disable psc2965 charge failed\n");
*/

		ret = psc2965_update_bits(info, PSC2965_REG_1,
					  PSC2965_REG_CHG_MASK,
					  0x0 << PSC2965_REG_CHG_SHIFT);
		if (ret) {
			dev_err(info->dev, "disable psc2965 charge failed\n");
			return ;
		}
		...
		if (info->is_wireless_charge) {
			ret = psc2965_update_bits(info, PSC2965_REG_1,
						...
						0x0);
			if (ret)
				dev_err(info->dev, "disable psc2965 charge en failed\n");
		}
	}
				  ...
				  u32 limit_cur)
{
	...
	//qzhu note: need to judge usb plug out or not
	if(info->usb_phy->chg_type == SDP_TYPE && reg_val <= 20)
	{
		reg_val = 20;
	}
				      ...
				      int val, u32 input_vol, bool bat_present)
{
	...
	if (val == CM_FAST_CHARGE_OVP_ENABLE_CMD) {
		...
		if (info->role == PSC2965_ROLE_MASTER_DEFAULT) {
			if (input_vol > PSC2965_FAST_CHARGER_VOLTAGE_MAX)
				info->need_disable_Q1 = true;
		}
	} else if ((val == false) &&
		   (info->role == PSC2965_ROLE_MASTER_DEFAULT)) {
		if (input_vol > PSC2965_NORMAL_CHARGER_VOLTAGE_MAX)
			info->need_disable_Q1 = true;
	}
					    ...
					    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CALIBRATE:
		if (info->role == PSC2965_ROLE_MASTER_DEFAULT) {
			...
			if (ret) {
				dev_err(info->dev, "get psc2965 charge status failed\n");
				goto out;
			}
...
static const struct power_supply_desc psc2965_charger_desc = {
	.name			= "psc2965_charger",
...
static int
psc2965_charger_register_vbus_regulator(struct psc2965_charger_info *info)
{
		...
		const struct i2c_device_id *id)
{
	 ...
	 */
	if (info->role == PSC2965_ROLE_MASTER_DEFAULT) {
		ret = psc2965_charger_register_vbus_regulator(info);
		if (ret) {
