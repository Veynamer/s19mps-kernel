...
#define SGM41511_PD_HARD_RESET_MS		500
#define SGM41511_PD_RECONNECT_MS			3000
...
static struct sgm41511_charger_reg_tab reg_tab[SGM41511_REG_NUM + 1] = {
	...
	{7, SGM41511_REG_7, "IINDET_EN/TMR2X_EN/BATFET_DIS/JEITA_VSET/BATFET_DLY/BATFET_RST_EN/VDPM_BAT_TRACK"},
	...
	{10, SGM41511_REG_A, "VBUS_GD/VINDPM_STAT/IINDPM_STAT/TOPOFF_ACTIVE/ACOV_STAT/VINDPM_INT_ MASK/IINDPM_INT_ MASK"},
	{11, SGM41511_REG_B, "REG_RST/PN/DEV_REV"},
	{12, 0, "null"},
};

static unsigned short s_level_delay_time[3]  ; // aw3632 = {2,2,510};
static void revo_set_otg_en(struct sgm41511_charger_info *info, int enable)
{
	if (gpio_is_valid(info->gpio_otg)) {
		dev_info(info->dev, "%s:%d\n", __func__, enable);

		if (enable) {
			gpio_set_value(info->gpio_otg, 1);
			if(s_level_delay_time[0])
				 udelay(s_level_delay_time[0]);
			gpio_set_value(info->gpio_otg, 0);
			if(s_level_delay_time[1])
				 udelay(s_level_delay_time[1]);
			gpio_set_value(info->gpio_otg, 1);
		} else {
			gpio_set_value(info->gpio_otg, 0);
			if(s_level_delay_time[2])
				 udelay(s_level_delay_time[2]);
		}
	}
}
...
/*
	revo :  revo_otg_init
*/
static int revo_otg_init(struct sgm41511_charger_info *info)
{
	int ret;
	dev_info(info->dev, "%s\n", __func__);
	ret = devm_gpio_request(info->dev,
			info->gpio_otg, "otg ext power detect");
	if (ret < 0) {
		dev_err(info->dev, "<revo otg> musb request otg-gpio error \n");
		return ret;
	}
	ret = gpio_direction_output(info->gpio_otg, 0);
	if (ret < 0)
		dev_err(info->dev, "<revo otg> musb direction output error \n");

	return ret;
}

/*
	revo :  revo_get_gpio_otg
*/
static int revo_get_gpio_otg(struct device_node *node)
{
	int gpio_otg  ;
	const unsigned short aw3632_delay_time[3] = {2,2,510};
	memset(s_level_delay_time,0,sizeof(s_level_delay_time));
	if (node) {
		gpio_otg = of_get_named_gpio(node, "revo,otg-gpio", 0);
		if( gpio_is_valid(gpio_otg) ){
			if(of_property_read_bool(node, "revo,otg_gpio-aw3632")){
				int i ;
				for(i=0 ; i< sizeof(s_level_delay_time)/sizeof(s_level_delay_time[0]);i++)
					s_level_delay_time[i] = aw3632_delay_time[i];
				printk("<revo otg>  [func:%s] OTG_GPIO_chip aw3632 \n",__func__);
			}
			printk("<revo otg>  [func:%s] gpio_otg=%d \n",__func__, gpio_otg);
			return  gpio_otg ;
		}
	} else {
		printk("<revo otg>  [func:%s] input parameters cannot be NULL \n",__func__);
	}
	return -1 ;
}
...
{
	u8 reg_val;
	printk("sgm41511: %s info->role=%d vol=%d \n",__func__,info->role,vol);
...
{
	u8 reg_val;
	printk("sgm41511: %s info->role=%d cur=%d \n",__func__,info->role,cur);
...
static int sgm41511_charger_hw_init(struct sgm41511_charger_info *info)
{
	...
	if (ret) {
		...
		dev_err(info->dev, "try sgm41513\n");
		sgm41513=1;
		ret = sgm41511_update_bits(info, SGM41511_REG_B,SGM41511_REG_RESET_MASK,SGM41511_REG_RESET_MASK); //D[7] is register reset
		if (ret) {
	...
	if (0){//(info->role == SGM41511_ROLE_MASTER_DEFAULT) {
	...
	ret = sgm41511_charger_set_limit_current(info, info->cur.unknown_cur);
	if (ret)
				   ...
				   u32 *charge_vol)
{
	...
	union power_supply_propval val;
	int ret;
	printk("sgm41511: %s info->role=%d charge_vol=%d \n",__func__,info->role,charge_vol);
...
#if 1
static void sgm41511_dump_chg_reg(struct sgm41511_charger_info *info) {
	int i = 0;
	int ret;
	unsigned char v = 0;
	for (i = 0; i < 0x0b; i++) {
		ret = sgm41511_read(info, i, &v);
		if (ret < 0) {
			dev_err(info->dev, "read [%d] error", i);
		}
		dev_info(info->dev, "%s reg[%02x] = %02x\n",__func__, i, v);
	}
}
#endif
...
#if 1
  sgm41511_dump_chg_reg(info);
#endif

	return ret;
}

static void sgm41511_charger_stop_charge(struct sgm41511_charger_info *info, bool present)
{
	int ret;
	printk("sgm41511: %s info->role=%d \n",__func__,info->role);
				       ...
				       u32 cur)
{
	...
	printk("sgm41511: %s info->role=%d reg_val=0x%x \n",__func__,info->role,reg_val);
...
static bool sgm41511_charger_get_power_path_status(struct sgm41511_charger_info *info)
{
	...
	ret = sgm41511_read(info, SGM41511_REG_0, &value);
	if (ret < 0) {
...
static int sgm41511_charger_set_power_path_status(struct sgm41511_charger_info *info, bool enable)
{
	int ret = 0;
	u8 value = 0x1;
	...
	ret = sgm41511_update_bits(info, SGM41511_REG_0,
				  SGM41511_REG_EN_HIZ_MASK,
				  value << SGM41511_REG_EN_HIZ_SHIFT);
	if (ret)
				      ...
				      int val, u32 input_vol, bool bat_present)
{
	...
	if (!val && info->charging) {
		...
		sgm41511_charger_stop_charge(info, bat_present);
		info->charging = false;
...
static int sgm41511_charger_pd_extcon_event(struct notifier_block *nb,
					   unsigned long event, void *param)
{
	struct sgm41511_charger_info *info =
		container_of(nb, struct sgm41511_charger_info, pd_extcon_nb);
	...
	schedule_delayed_work(&info->pd_hard_reset_work,
			      msecs_to_jiffies(SGM41511_PD_HARD_RESET_MS));

	return NOTIFY_OK;
}

static void sgm41511_charger_detect_pd_extcon_status(struct sgm41511_charger_info *info)
{
...
static void sgm41511_charger_pd_hard_reset_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct sgm41511_charger_info *info = container_of(dwork,
			struct sgm41511_charger_info, pd_hard_reset_work);
...
static int sgm41511_charger_register_pd_extcon(struct device *dev,
					      struct sgm41511_charger_info *info)
{
	int ret = 0;
	...
	INIT_DELAYED_WORK(&info->pd_hard_reset_work, sgm41511_charger_pd_hard_reset_work);
	...
	if (of_property_read_bool(dev->of_node, "extcon")) {
		...
		info->pd_extcon_nb.notifier_call = sgm41511_charger_pd_extcon_event;
...
static void sgm41511_charger_typec_extcon_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct sgm41511_charger_info *info =
		container_of(dwork, struct sgm41511_charger_info, typec_extcon_work);
...
static int sgm41511_charger_typec_extcon_event(struct notifier_block *nb,
					      unsigned long event, void *param)
{
	struct sgm41511_charger_info *info =
		container_of(nb, struct sgm41511_charger_info, typec_extcon_nb);
...
static int sgm41511_charger_register_typec_extcon(struct device *dev,
						 struct sgm41511_charger_info *info)
{
	int ret = 0;
	...
	INIT_DELAYED_WORK(&info->typec_extcon_work, sgm41511_charger_typec_extcon_work);
	...
	if (of_property_read_bool(dev->of_node, "extcon")) {
		...
		info->typec_extcon_nb.notifier_call = sgm41511_charger_typec_extcon_event;
		ret = devm_extcon_register_notifier_all(dev,
							info->typec_extcon,
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if (val->intval == CM_POWER_PATH_ENABLE_CMD) {
			ret = sgm41511_charger_set_power_path_status(info, true);
			break;
		} else if (val->intval == CM_POWER_PATH_DISABLE_CMD) {
			ret = sgm41511_charger_set_power_path_status(info, false);
			break;
		}

		ret = sgm41511_charger_set_status(info, val->intval, input_vol, bat_present);
		if (ret < 0)
		...
		const struct i2c_device_id *id)
{
	...
	ret = sgm41511_charger_register_pd_extcon(info->dev, info);
	if (ret) {
		dev_err(info->dev, "failed to register pd extcon\n");
		return -EPROBE_DEFER;
	}

	ret = sgm41511_charger_register_typec_extcon(info->dev, info);
	if (ret) {
	...
	if (1) {//(info->role == SGM41511_ROLE_SLAVE)
		info->psy_usb = devm_power_supply_register(dev,
	...
	/* revo  gpio_otg  */
	info->gpio_otg = revo_get_gpio_otg(info->dev->of_node);
	if ( info->gpio_otg != -1 ){
		ret = revo_otg_init(info);
		if (ret < 0)
			dev_err(dev, "<revo otg> fail to init gpio otg (revo,otg-gpio)\n");
	}
	mutex_unlock(&info->lock);
	sgm41511_charger_detect_status(info);
	sgm41511_charger_detect_pd_extcon_status(info);
	
	//slave charger config chg_en enable
	ret = sgm41511_update_bits(info, SGM41511_REG_1, SGM41511_REG_CHG_MASK, 0x1 << SGM41511_REG_CHG_SHIFT);
	if (ret) {
		dev_err(info->dev, "enable sgm41511 charge en failed\n");
	}
