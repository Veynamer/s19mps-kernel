...
static int sprd_panel_unprepare(struct drm_panel *p)
{
	...
	//revo lcd 3v3 1v8 gpio add start
	if (panel->info.pwr_1v8_gpio) {
		gpiod_direction_output(panel->info.pwr_1v8_gpio, 0);
	}	
	if (panel->info.pwr_3v3_gpio) {
		gpiod_direction_output(panel->info.pwr_3v3_gpio, 0);
	}
	//revo lcd 3v3 1v8 gpio add end
...
static int sprd_panel_prepare(struct drm_panel *p)
{
	...
	//revo lcd 3v3 1v8 gpio add start
	if (panel->info.pwr_3v3_gpio) {
		gpiod_direction_output(panel->info.pwr_3v3_gpio, 1);
	}
	if (panel->info.pwr_1v8_gpio) {
		gpiod_direction_output(panel->info.pwr_1v8_gpio, 1);
	}
	//revo lcd 3v3 1v8 gpio add end
...
{
	//revo lcd 3v3 1v8 gpio add start	
	panel->info.pwr_3v3_gpio = devm_gpiod_get_optional(dev,"3v3", GPIOD_ASIS);
	if (IS_ERR_OR_NULL(panel->info.pwr_3v3_gpio))
		DRM_WARN("can't get panel 3v3 gpio: %ld\n", PTR_ERR(panel->info.pwr_3v3_gpio));
		
	panel->info.pwr_1v8_gpio = devm_gpiod_get_optional(dev, "1v8", GPIOD_ASIS);
	if (IS_ERR_OR_NULL(panel->info.pwr_1v8_gpio))
		DRM_WARN("can't get panel 1v8 gpio: %ld\n",PTR_ERR(panel->info.pwr_1v8_gpio));
	//revo lcd 3v3 1v8 gpio add end
