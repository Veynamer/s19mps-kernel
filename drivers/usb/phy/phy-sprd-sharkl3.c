...
//revo modify:improve usb driver strength:voltage 2.7v to 3.3v for calibration mode
#define FULLSPEED_USB33_TUNE		3300000//2700000
...
// extern struct gpio_desc *usb_det_desc;
struct platform_device *musb_pdev;
static void usb_phy_set_charger_state_f(struct usb_phy *usb_phy,
				  ...
				  unsigned long event, void *data)
{
	...
	if(musb_pdev) {
		glue = platform_get_drvdata(musb_pdev);
		if(glue) {
			pr_err("%s:is_docking_online:%d\n",__func__, glue->is_docking_online);
		}
	}

	// pr_err("%s, plug in, event = %d, usb_det = %d, otg_mode=%d\n", __func__, event, gpiod_get_value_cansleep(usb_det_desc),otg_mode);
	pr_err("%s, plug in, event = %d\n", __func__, event);
#endif

	if (phy->is_host) {
		dev_info(phy->dev, "USB PHY is host mode\n");
	#ifdef PRJ_FEATURE_H_BOARD_DOCKING_SUPPORT
		if (glue && glue->is_docking_online && event) {
			pr_err("%s, in usb host mode:keyboard, start charge, according to real type!!!\n", __func__);
			//sc27xx_bc1p2_enable(phy->pmic, 1);
			type = sc27xx_charger_detect(phy->pmic);
			pr_err("%s, charge type = 0x%x\n", __func__, type);
			if (type)
					usb_phy->chg_type = type;
			else
					usb_phy->chg_type = SDP_TYPE;

			usb_phy_set_charger_state_f(usb_phy, USB_CHARGER_PRESENT);
		}

		//if (glue->is_docking_online && !event) {
		if (!event) {
			pr_err("%s, in usb host mode:keyboard, stop charge!!!\n", __func__);
			usb_phy_set_charger_state_f(usb_phy, USB_CHARGER_ABSENT);
		}
	#endif
		return 0;
	}
	...
	if (event) {
	#ifdef PRJ_FEATURE_H_BOARD_DOCKING_SUPPORT
		msleep(50);// sw debounce time
		// pr_err("%s, plug in, usb_det = %d\n", __func__, gpiod_get_value_cansleep(usb_det_desc));
		pr_err("%s, plug in\n", __func__);
...
static int phy_sprd_sharkl3_notify_connect(struct usb_phy *phy,
...
static int phy_sprd_sharkl3_notify_disconnect(struct usb_phy *phy,
                                      enum usb_device_speed speed)
{
	//struct sprd_glue *glue;
	...
	//if(musb_pdev) {
	//	glue = platform_get_drvdata(musb_pdev);
	//	if(glue && glue->is_docking_online) {
	//		pr_err("%s:is_docking_online:%d\n",__func__, glue->is_docking_online);
	//		schedule_delayed_work(&glue->docking_boot_work, msecs_to_jiffies(500));
	//	}
	//}
	pr_err("%s:\n",__func__);
	return 0;
...
static int sprd_hsphy_probe(struct platform_device *pdev)
{
...
#ifdef PRJ_FEATURE_H_BOARD_DOCKING_SUPPORT
	phy->phy.notify_connect     = phy_sprd_sharkl3_notify_connect;
	phy->phy.notify_disconnect  = phy_sprd_sharkl3_notify_disconnect;
#endif
	otg->usb_phy = &phy->phy;
	...
	if (extcon_get_state(phy->phy.edev, EXTCON_USB) > 0)
		usb_phy_set_charger_state_f(&phy->phy, USB_CHARGER_PRESENT);
#else
	if (extcon_get_state(phy->phy.edev, EXTCON_USB) > 0)
...
#ifdef PRJ_FEATURE_H_BOARD_DOCKING_SUPPORT
	musb_pdev = find_sprd_musb_dev(dev);
	if(!musb_pdev) {
...
#ifdef PRJ_FEATURE_H_BOARD_DOCKING_SUPPORT
module_init(sprd_hsphy_driver_init);
#else
